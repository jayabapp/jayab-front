...
export const captureToken = (raw: string) => {
  ...
  try {
    ...
    return { body: raw, token: undefined as string | undefined };
  }
  ...
  if (!key) return { body: raw, token: undefined as string | undefined };
  const token: string = payload[key];
  delete payload.access_token;
  delete payload.auth_token;
  payload.needs_registration = key === "auth_token";
  return { body: JSON.stringify(parsed), token: token as string | undefined };
};
