...
const OwnerPhotoUpgradeRequestPage = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [PhotoUpgradeService.OWNER_PHOTO_UPGRADE_REQUEST_CACHEKEY, params.id],
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
              <div className="min-w-0">
                <h1 className="line-clamp-1 text-base font-medium md:text-xl">{data?.property?.title || "اقامتگاه"}</h1>
                <p className="mt-1 text-xs text-gray-500">کد {data?.property?.code || data?.property_id}</p>
              </div>
              {data?.status ? <StatusShower data={data.status} containerClass="shrink-0 !px-2 !py-1" /> : <></>}
            </div>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-2 lg:grid-cols-2 ">
          <SummaryItem title="شماره درخواست" value={data?.id || "-"} />
          <SummaryItem title="تعداد عکس" value={`${data?.image_count || data?._count?.items || 0} عکس`} />
          <SummaryItem title="قیمت هر عکس" value={`${numberWithCommas(data?.price_per_image)} تومان`} />
          <SummaryItem title="مبلغ کل" value={`${numberWithCommas(data?.total_amount)} تومان`} />
          <SummaryItem
            title="ثبت درخواست"
            value={data?.created_at ? moment(data.created_at).format("HH:mm - jYYYY/jMM/jDD") : "-"}
          />
          <SummaryItem
            title="تکمیل درخواست"
            value={data?.completed_at ? moment(data.completed_at).format("HH:mm - jYYYY/jMM/jDD") : "-"}
          />
        </div>
      </div>
      <div className=" grid grid-cols-1 lg:grid-cols-2 gap-3">
        {data?.items && data.items.length > 0 ? (
          data.items.map((item, index) => (
            <PhotoUpgradeImagePair item={item} index={index} key={`photoUpgradeRequestItem${item.id}`} />
          ))
        ) : (
          <div className="white-card text-center text-sm text-gray-500">تصویری برای این درخواست ثبت نشده است.</div>
        )}
      </div>
    </div>
  );
