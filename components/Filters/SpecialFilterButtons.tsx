...
const SpecialFilterButon = ({ item, isChecked, cb }: { item: any; isChecked: boolean; cb: () => void | null }) => {
  return (
    <div
      onClick={cb}
      className={`rounded-full !w-auto   ${!!isChecked ? " border-brand-600  bg-brand-600/5 text-brand-600" : "  opacity-70"}  transition-all  cursor-pointer  gap-0   py-1 h-[1.625rem] pl-2 pr-1 flex items-center justify-center border      text-xs `}
    >
      ...
      {!!isChecked ? (
        <>
          <div className=" cursor-pointer w-4 h-4  mr-2 aspect-square rounded-full border border-brand-600 flex items-center justify-center">
