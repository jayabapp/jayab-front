...
import type { OwnerCommissionModalProps } from "@/types/components/modules/owner-property";
...
const OwnerCommissionAllDaysModal = ({
  show,
  onHide,
  property,
}: OwnerCommissionModalProps) => {
  const {
  ...
  const key = `${property?.id ?? ""}|${show}`;
  let current = draft;
  if (current.key !== key)
    current = { key, value: property?.advisor_commission || 0 };
  if (current !== draft) setDraft(current);

  const onSubmit = () => {
    if (isPending) return;
    mutate(
      { advisor_commission: current.value, property_id: property?.id },
      { onSuccess: onHide },
    );
  };
...
export default OwnerCommissionAllDaysModal;
