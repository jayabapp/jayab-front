...
function ModalSearchCities({ showModal, onHide }: ModalSearchCitiesProps) {
  ...
  return (
    <Modal
      options={{
        containerClass: "mx-auto md:my-10 w-full h-screen md:h-full max-w-lg md:rounded-2xl overflow-y-scroll bg-white dark:bg-zinc-900",
      }}
      show={showModal}
      onHide={onHide}
    >
      <div className="h-auto">
        <div className="flex flex-col h-auto">
          <div className="flex items-center justify-center sticky select-none z-[40] shadow-md bg-white w-full transition-all top-0 h-16 px-6 py-4">
            <button onClick={onHide} className="text-2xl text-gray-500 absolute left-6 top-6">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <ul className="flex items-center gap-1 list-none p-0 m-0 text-xs text-[#9296A0]">
                          {(item.child?.length ?? 0) > 0 &&
                            item.child?.map((city: City, cityIndex: number) => (
                              <li key={city.id}>
                                {city.title}
                                {cityIndex < (item.child?.length ?? 0) - 1 && " -"}
                              </li>
                            ))}
                        </ul>
                      </div>
