...
const RegionButton = ({
  ...
  return (
    ...
    >
      <p className="text-xs pr-1  shrink-0 ">{!isEmpty(regionsIds) ? _STRINGS.LOCAL : _STRINGS.SELECT_LOCAL} </p>

      {!isEmpty(regionsIds) ? <p className="shrink-0"> {`(${regionsIds?.length} مورد) `} </p> : ""}
      {!isEmpty(regionsIds) ? (
        <div
          onClick={(e) => {
            e.preventDefault();
